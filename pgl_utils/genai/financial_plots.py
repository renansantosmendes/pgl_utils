...
def plot_raw_data(
    close_prices: pd.Series, log_returns: np.ndarray, ticker: str
) -> None:
...
def plot_projections_and_distributions(
    ...
    axes[0].axvline(
        0, color="black", linestyle="--", linewidth=1, alpha=0.6, label="Today"
    )
...
def plot_individual_paths(
    ...
    fig, axes = plt.subplots(
        len(generated_paths), 1, figsize=(14, 4 * len(generated_paths))
    )
    ...
    plt.suptitle(
        f"{ticker} — Real History + Each Generated Path", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    plt.show()


def plot_return_statistics(
    log_returns: np.ndarray, generated_paths: list[np.ndarray]
) -> None:
...
def plot_volatility_cone(
    ...
    ax.plot(
        np.arange(-recent_history_window, 0),
        close_prices.values[-recent_history_window:],
        color="steelblue",
        linewidth=2,
        label="Real history",
    )
    ...
    ax.fill_between(
        future_days, p5, p95, alpha=0.15, color="darkorange", label="90% of scenarios"
    )
    ax.fill_between(
        future_days, p25, p75, alpha=0.30, color="darkorange", label="50% of scenarios"
    )
    ax.plot(
        future_days,
        p50,
    ...
    ax.set_title(
        f"{ticker} — VAE Volatility Cone ({num_scenarios} scenarios)", fontsize=14
    )
    ax.set_xlabel("Days (0 = today)")
    ax.set_ylabel("Price (USD)")
    ax.legend()
